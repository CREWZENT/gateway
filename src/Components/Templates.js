...
class DefaultName extends Component {
    ...
    async createQuiz(quiz) {
        ...
        if (Number(depositValue) < state.balance) {
            ...
            try {
                ...
                const tx2 = await state.HrTest.methods.createQuiz(quiz.name, BigNumber(depositValue * 10 ** 18).toString(10)).send();
                if (tx1.blockHash && tx2.blockHash) {
                    const quizId = tx2.events.CreateQuiz.returnValues.quizId;
                    this.setState({ creating: true, progress: 0, questionLength: quiz.questionsList.length });
    ...
    render() {
        const { state } = this.props;
        const { creating, progress, questionLength } = this.state;
            ...
            <div className="template container">
                {
                    ...
                    <div className="row">
                        <div className="text-center mt-5 mx-auto">
                            ...
                            Creating Room With {questionLength} Questions
                            <div className="progress">
                                <div className="progress-bar bg-success" role="progressbar" style={{ "width": 5 + progress / questionLength * 95 + "%" }} aria-valuenow={process} aria-valuemin="0" aria-valuemax="100"></div>
                            </div>
                        </div>
                    </div>
                }
                ...
                {
                    state.Gateway && creating === false &&
                    <div className="row">
                        <div className="col-md-12 mt-5">
                            <h1><b>Choose quiz</b></h1>
                        </div>
                        {
                            this.state.quizsList.map((quiz, x) => {
                                return (
                                    <div key={x} className="col-sm-4 mb-4">
                                        <div className="card quizCard">
                                            <img className="card-img-top" src={require(`../assets/img/${quiz.image}`)} alt="" style={{ maxHeight: "180px" }} />
                                            <div className="card-body">
                                                <h5 className="card-title"><b>{quiz.name} Quiz</b></h5>
                                                <p className="small">{quiz.questionsList.length} Questions</p>
                                                <p className="card-text">{quiz.description}</p>
                                                <button className="btn btn-primary" onClick={() => this.createQuiz(quiz)}>Create Room</button>
                                                {/* <a className="btn btn-outline-success ml-2" href="/analytic">Analytic</a> */}
                                            </div>
                                        </div>
                                    </div>
                                );
